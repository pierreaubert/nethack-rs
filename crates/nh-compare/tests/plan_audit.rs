...
fn audit_0_2_dual_orchestrator_compares_c_vs_rust() {
    ...
    let has_independent_compare =
        orchestrator.contains("compare_states") && !orchestrator.contains("c_wrapper.set_state");
...
fn audit_2_1_monster_field_parser() {
    ...
    let returns_zeros = parser.contains("symbol: ' '")
        && parser.contains("level: 0")
        && parser.contains("ac: 0")
        && parser.contains("mr: 0");
    ...
    let fields_needed = [
        (
            "symbol",
            parser.contains("symbol") && !parser.contains("symbol: ' '"),
        ),
        ...
        (
            "attacks",
            parser.contains("ATTK") || parser.contains("parse_attack"),
        ),
...
fn audit_2_2_object_field_comparison() {
    ...
    let compares_nutrition =
        static_data.contains("nutrition") && static_data.contains("c_nutrition");
...
fn audit_implementation_line_counts() {
    let modules = [
        ...
        (
            "3.1 mkobj.rs",
            &format!("{}/object/mkobj.rs", NH_CORE_SRC),
            2969,
            80,
        ),
        (
            "3.2 objname.rs",
            &format!("{}/object/objname.rs", NH_CORE_SRC),
            4300,
            50,
        ),
        (
            "3.3 inventory.rs",
            &format!("{}/object/inventory.rs", NH_CORE_SRC),
            4479,
            30,
        ),
        (
            "4.1 eat.rs",
            &format!("{}/action/eat.rs", NH_CORE_SRC),
            3352,
            50,
        ),
        (
            "4.2 apply.rs",
            &format!("{}/action/apply.rs", NH_CORE_SRC),
            3811,
            30,
        ),
        (
            "4.3 pickup.rs",
            &format!("{}/action/pickup.rs", NH_CORE_SRC),
            3272,
            30,
        ),
        (
            "4.4 wear.rs",
            &format!("{}/action/wear.rs", NH_CORE_SRC),
            2846,
            30,
        ),
        (
            "4.5 trap.rs",
            &format!("{}/action/trap.rs", NH_CORE_SRC),
            5476,
            10,
        ),
        (
            "5.1 zap.rs",
            &format!("{}/magic/zap.rs", NH_CORE_SRC),
            5354,
            30,
        ),
        (
            "5.2 scroll.rs",
            &format!("{}/magic/scroll.rs", NH_CORE_SRC),
            2652,
            30,
        ),
        (
            "5.3 potion.rs",
            &format!("{}/magic/potion.rs", NH_CORE_SRC),
            2412,
            30,
        ),
        (
            "5.4 pray.rs",
            &format!("{}/action/pray.rs", NH_CORE_SRC),
            2302,
            10,
        ),
        (
            "5.6 shop.rs",
            &format!("{}/dungeon/shop.rs", NH_CORE_SRC),
            4973,
            15,
        ),
        (
            "6.1 monst.rs",
            &format!("{}/monster/monst.rs", NH_CORE_SRC),
            4264,
            20,
        ),
        (
            "8.2 special_level.rs",
            &format!("{}/dungeon/special_level.rs", NH_CORE_SRC),
            6059,
            20,
        ),
        (
            "10.1 options.rs",
            &format!("{}/world/options.rs", NH_CORE_SRC),
            6944,
            10,
        ),
        (
            "10.3 gameloop.rs",
            &format!("{}/gameloop.rs", NH_CORE_SRC),
            6117,
            20,
        ),
    ];
    ...
    for (desc, path, c_lines, min_pct) in &modules {
        ...
        let pct = if *c_lines > 0 {
            rust_lines * 100 / c_lines
        } else {
            0
        };
        ...
        println!(
            "{:<25} {:>8} {:>8} {:>5}% {:>7}% {}",
            desc, rust_lines, c_lines, pct, min_pct, status
        );
...
fn audit_missing_modules() {
    let missing = [
        (
            "5.5 artifact effects",
            &format!("{}/object/artifact.rs", NH_CORE_SRC),
            2205,
        ),
        (
            "6.2 makemon.rs",
            &format!("{}/monster/makemon.rs", NH_CORE_SRC),
            2318,
        ),
        (
            "6.3 polymorph.rs",
            &format!("{}/magic/polymorph.rs", NH_CORE_SRC),
            1907,
        ),
        (
            "6.4 detect.rs",
            &format!("{}/magic/detect.rs", NH_CORE_SRC),
            2032,
        ),
        (
            "10.2 death.rs",
            &format!("{}/player/death.rs", NH_CORE_SRC),
            2292,
        ),
    ];
    ...
    println!(
        "\nSTATUS: {} modules still missing",
        missing
            .iter()
            .filter(|(_, p, _)| !file_exists(p) || count_lines(p) < 50)
            .count()
    );
...
fn audit_7_command_enum_completeness() {
    ...
    println!(
        "Missing: {}/{}",
        required_commands.len() - present_count,
    ...
    println!(
        "\nExisting commands with 'not yet implemented': {}",
        has_stubs
    );
    ...
    println!(
        "STATUS: 0/{} required commands added to enum",
        required_commands.len()
    );
}
...
fn audit_9_replay_parity() {
    ...
    println!(
        "  9.2 500-turn replays: {}",
        if has_500_turn { "DONE" } else { "NOT DONE" }
    );
    println!(
        "  9.3 5000-turn replays: {}",
        if has_5000_turn { "DONE" } else { "NOT DONE" }
    );
    println!(
        "  9.4 Ascension replay: {}",
        if has_ascension { "DONE" } else { "NOT DONE" }
    );
...
fn audit_overall_summary() {
    println!(
        "\n{} CONVERGENCE PLAN AUDIT SUMMARY {}",
        "=".repeat(20),
        "=".repeat(20)
    );
    ...
    let items = [
        (
            "0.2 Dual C/Rust orchestrator",
            "NOT DONE",
            "C FFI init crashes; orchestrator force-syncs C to Rust",
        ),
        (
            "1.2 RNG call-site tracing C vs Rust",
            "PARTIAL",
            "Tracing exists but only compares Rust-to-Rust",
        ),
        (
            "2.1 Monster field comparison (380+)",
            "NOT DONE",
            "Parser returns hardcoded zeros for all fields",
        ),
        (
            "2.2 Object field comparison",
            "PARTIAL",
            "Names and weights only; missing cost/material/nutrition/etc",
        ),
        (
            "2.3 Artifact field comparison",
            "PARTIAL",
            "Names and cost only; missing alignment/attack/defense/invoke",
        ),
        (
            "2.4 Role/race stat/inventory comparison",
            "PARTIAL",
            "Names only; missing starting inventory/stats/alignment",
        ),
        (
            "3.1 mkobj.c port (2,969 lines)",
            "PARTIAL",
            "1,441 lines (48%); missing mkgold(), mkcorpstat()",
        ),
        (
            "3.2 objnam.c port (4,300 lines)",
            "PARTIAL",
            "660 lines (15%); limited xname/doname",
        ),
        (
            "3.3 invent.c port (4,479 lines)",
            "PARTIAL",
            "429 lines (10%); missing getobj(), identify()",
        ),
        (
            "4.2 apply.c port (3,811 lines)",
            "NOT DONE",
            "292 lines (8%); only 7 tools implemented",
        ),
        (
            "4.5 trap.c port (5,476 lines)",
            "NOT DONE",
            "57 lines (1%); only 3 trap effects",
        ),
        (
            "5.4 pray.c port (2,302 lines)",
            "NOT DONE",
            "65 lines (3%); stub implementation",
        ),
        (
            "5.5 artifact.c port (2,205 lines)",
            "MISSING",
            "File does not exist",
        ),
        (
            "5.6 shk.c trading system (4,973)",
            "NOT DONE",
            "386 lines (8%); no buy/sell/pay",
        ),
        (
            "6.2 makemon.c (2,318 lines)",
            "MISSING",
            "File does not exist",
        ),
        (
            "6.3 polyself.c (1,907 lines)",
            "MISSING",
            "File does not exist",
        ),
        (
            "6.4 detect.c (2,032 lines)",
            "MISSING",
            "File does not exist",
        ),
        (
            "7.1 17 action commands",
            "NOT DONE",
            "0/17 added to Command enum",
        ),
        (
            "7.2 10 info commands",
            "NOT DONE",
            "0/10 added to Command enum",
        ),
        (
            "7.3 7 wizard commands",
            "NOT DONE",
            "0/7 added to Command enum",
        ),
        (
            "8.2 Special levels (Sokoban etc)",
            "PARTIAL",
            "966/6,059 lines (16%); no Sokoban data",
        ),
        (
            "9.1-9.3 C vs Rust turn comparison",
            "NOT DONE",
            "Rust-only; no 500/5000 turn tests",
        ),
        ("9.4 Ascension replay", "NOT DONE", "Not implemented"),
        ("10.1 options.c (6,944 lines)", "NOT DONE", "479 lines (7%)"),
        (
            "10.2 end.c death handling (2,292)",
            "PARTIAL",
            "589 lines (26%) in endgame.rs",
        ),
    ];
    ...
    println!("{}", "-".repeat(100));
    println!(
        "DONE: {}  PARTIAL: {}  NOT DONE: {}  MISSING: {}",
        done, partial, not_done, missing
    );
