...
fn run_room_parity_test(seed: u64) {
    ...
    for (i, r) in c_rooms.iter().enumerate() {
        println!(
            ...
            r["lx"],
            r["ly"],
            r["hx"].as_i64().unwrap() - r["lx"].as_i64().unwrap() + 1,
            r["hy"].as_i64().unwrap() - r["ly"].as_i64().unwrap() + 1,
            r["type"]
        );
    }
    ...
    for entry in &trace {
        writeln!(
            trace_file,
            "RS: {}({}) = {} (raw={})",
            entry.func, entry.arg, entry.result, entry.raw
        )
        .unwrap();
    }
    println!(
        "Rust: wrote {} trace entries to {}",
        trace.len(),
        trace_path
    );
    ...
    let rs_rooms: Vec<_> = fresh_level
        ...
        .collect();
    let c_rooms: Vec<_> = c_rooms
        .iter()
        .filter(|r| r["type"].as_i64().unwrap_or(0) != 4)
        .collect();

    println!(
        "Rust generated {} rooms ({} total incl. vault)",
        rs_rooms.len(),
        fresh_level.rooms.len()
    );
    ...
    for (i, c_room) in c_rooms.iter().enumerate() {
        ...
        assert_eq!(
            rs_lx,
            c_room["lx"].as_i64().unwrap(),
            "Room {} lx mismatch for seed {}",
            i,
            seed
        );
        assert_eq!(
            rs_hx,
            c_room["hx"].as_i64().unwrap(),
            "Room {} hx mismatch for seed {}",
            i,
            seed
        );
        assert_eq!(
            rs_ly,
            c_room["ly"].as_i64().unwrap(),
            "Room {} ly mismatch for seed {}",
            i,
            seed
        );
        assert_eq!(
            rs_hy,
            c_room["hy"].as_i64().unwrap(),
            "Room {} hy mismatch for seed {}",
            i,
            seed
        );
    }
    ...
    if !mismatches.is_empty() {
        println!(
            "Cell mismatches for seed {}: {} total",
            seed,
            mismatches.len()
        );
        // Group by type of mismatch
        let mut type_counts: std::collections::HashMap<(String, String), usize> =
