...
fn test_maze_parity_seeds() {
    for seed in [42, 123, 999, 1337, 2026] {
        ...
        let rs_passages = fresh_level
            .cells
            .iter()
            .flat_map(|col| col.iter())
            .filter(|cell| {
                cell.typ == nh_core::dungeon::CellType::Corridor
                    || cell.typ == nh_core::dungeon::CellType::Room
            })
            .count();
        ...
        assert_eq!(
            rs_passages, c_passages,
            "Maze passage count mismatch for seed {}",
            seed
        );
    }
}
