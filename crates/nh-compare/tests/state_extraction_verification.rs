...
fn test_rust_inventory_state_extraction_depth() {
    ...
    assert!(
        !inv.is_empty(),
        "Archeologist should have starting inventory"
    );

    for item in inv {
        // We want to ensure these fields are accessible and correctly populated
        println!(
            "Item Type: {}, Weight: {}, BUC: {:?}",
            item.object_type, item.weight, item.buc
        );
    }
}
...
fn test_rust_monster_state_extraction_depth() {
    ...
    for monster in monsters {
        ...
        println!(
            "Monster: {}, Pos: ({}, {}), Asleep: {}",
            monster.name, monster.x, monster.y, monster.state.sleeping
        );
