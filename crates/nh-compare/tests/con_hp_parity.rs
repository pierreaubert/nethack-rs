    ...
    for (role, role_name) in roles {
        ...
        for (race, race_name) in &races {
            ...
            if rust_state.player.hp_max != c_engine.max_hp() {
                println!(
                    "  !! HP Max mismatch for {} {} at seed 42. C: {}, Rust: {}",
                    role_name,
                    race_name,
                    c_engine.max_hp(),
                    rust_state.player.hp_max
            ...
            if rust_state.player.energy_max != c_engine.max_energy() {
                println!(
                    "  !! Energy Max mismatch for {} {} at seed 42. C: {}, Rust: {}",
                    role_name,
                    race_name,
                    c_engine.max_energy(),
                    rust_state.player.energy_max
