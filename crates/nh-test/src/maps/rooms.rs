    ...
    vec![
        GenerationComparison {
            ...
            notes: if c.max_rooms != rust.max_rooms {
                format!(
                    "Rust generates fewer rooms ({}  vs C's {})",
                    rust.max_rooms, c.max_rooms
                )
            } else {
                String::new()
            },
...
pub fn extract_c_room_constants() -> Option<super::room_types::CRoomConstants> {
    let global_h = Path::new(crate::c_source_parser::NETHACK_SRC).join("include/global.h");
...
mod tests {
    ...
    fn test_rust_room_constants() {
        ...
        println!(
            "  Room size: {}x{} to {}x{}",
            rust.min_room_size.0, rust.min_room_size.1, rust.max_room_size.0, rust.max_room_size.1
        );
    ...
    fn test_c_room_constants() {
        ...
        if let Some(c) = c {
            ...
            println!(
                "  Room size: {}x{} to {}x{}",
                c.min_room_width, c.min_room_height, c.max_room_width, c.max_room_height
            );
    ...
    fn test_missing_features() {
        ...
        let critical: Vec<_> = missing
            ...
            .collect();
        let high: Vec<_> = missing
            .iter()
            .filter(|f| f.priority == FeaturePriority::High)
            .collect();
        let medium: Vec<_> = missing
            .iter()
            .filter(|f| f.priority == FeaturePriority::Medium)
            .collect();
        let low: Vec<_> = missing
            .iter()
            .filter(|f| f.priority == FeaturePriority::Low)
            .collect();
    ...
    fn test_room_type_implementation_status() {
        ...
        let implemented_count = CRoomType::ALL
            .iter()
    ...
    fn test_special_room_spawn_requirements() {
        ...
        println!(
            "{:<20} {:>10} {:>15}",
            "Room Type", "Min Depth", "Probability"
        );
        println!("{}", "-".repeat(50));
