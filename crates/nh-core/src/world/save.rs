...
mod fs_impl {
    ...
    pub fn write_save_header(
        ...
        let header_json =
            serde_json::to_string(header).map_err(|e| SaveError::SerializeError(e.to_string()))?;
    ...
    pub fn get_save_info(path: &Path) -> Result<SaveInfo, SaveError> {
        let file = File::open(path).map_err(|e| SaveError::IoError(e.to_string()))?;
    ...
    pub fn list_saves() -> Result<Vec<SaveInfo>, SaveError> {
        ...
        let entries = std::fs::read_dir(&dir).map_err(|e| SaveError::IoError(e.to_string()))?;
