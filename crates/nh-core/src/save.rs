...
pub fn load_game(path: impl AsRef<Path>) -> Result<GameState, SaveError> {
    ...
    let mut state = save_file.state;

    // Restore priest data for current level after loading
    // Get the level info before iterating
    let shrine_level_id = priest::ShrineLevelId {
        dungeon: state.current_level.dlevel.dungeon_num as u8,
        level: state.current_level.dlevel.level_num as u8,
    };
...
pub struct BonesFile {
    ...
    pub level: crate::dungeon::Level,
    /// Ghost monster data (if applicable)
    pub ghost: Option<crate::monster::Monster>,
}
...
pub fn save_bones(
    ...
    ghost: Option<&crate::monster::Monster>,
) -> Result<(), SaveError> {
    let bones = BonesFile {
