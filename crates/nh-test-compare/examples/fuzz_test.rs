...
use nh_test_compare::orchestrator::{DualGameOrchestrator, OrchestratorConfig};
...
fn main() {
    ...
    loop {
        ...
        if sessions % 10 == 0 {
            ...
            print!("\rSessions: {} | Failures: {} | Turns: {} | Rate: {:.1} eps/sec ", 
                   sessions, failures, total_turns, sessions as f64 / elapsed);
        ...
        if !result.critical_differences.is_empty() {
            failures += 1;
            println!("\n\n[FAILURE] Seed {}: Found {} critical differences", seed, result.critical_differences.len());
