...
use candle_nn::{linear, Linear, Module, VarBuilder, VarMap, Optimizer, SGD};
...
impl ReplayMemory {
    fn new(capacity: usize) -> Self {
        Self { memory: Vec::with_capacity(capacity), capacity }
    }
...
impl QNetwork {
    fn new(input_size: usize, hidden_size: usize, output_size: usize, device: &Device) -> Self {
        ...
        Self { fc1, fc2, fc3, varmap, device: device.clone() }
    }
...
fn calculate_reward(old_state: &UnifiedGameState, new_state: &UnifiedGameState, _action: &GameAction, message: &str) -> f32 {
    let mut reward = 0.0f32;
    if new_state.is_dead { return -10.0; }
...
fn train_multithreaded(config: ConfigArgs, args: &Args) {
    ...
    for (ep, reward, _length, transitions) in episodes_rx {
        ...
        if total_episodes % 10 == 0 && total_episodes >= 100 {
            ...
            if !samples.is_empty() {
                ...
                for t in samples.iter() {
                    ...
                    let max_next_q = if t.done { 0.0 } else { next_q_values.iter().fold(f32::MIN, |a, b| a.max(*b)) };
                    targets.push(t.reward + config.gamma as f32 * max_next_q);
                }

                drop(memory);

                let states_tensor = Tensor::from_slice(&states, (batch_size, STATE_SIZE), &device).unwrap();
                ...
                let loss = selected_q.sub(&targets_tensor).unwrap().sqr().unwrap().mean_all().unwrap();
        ...
        if total_episodes % 50 == 0 {
            ...
            let avg_batch: f64 = batch_rewards.iter().map(|&r| r as f64).sum::<f64>() / batch_rewards.len() as f64;
            ...
            if matches!(log_level, LogLevel::Info | LogLevel::Debug) {
                println!("Episode {:5}: {:.1} eps/s | Avg: {:7.2} | Deaths: {} | Eps: {:.3}",
                    total_episodes, eps_per_sec, avg_batch, deaths,
                    (config.exploration_start * config.exploration_decay.powf(total_episodes as f64)).max(config.exploration_end));
            }
        ...
        if total_episodes % config.save_interval == 0 {
            ...
            if matches!(log_level, LogLevel::Info | LogLevel::Debug) {
                println!("  -> Saved checkpoint at episode {} (avg reward: {:.2})", total_episodes, avg_batch);
            }
        }
    ...
    if matches!(log_level, LogLevel::Info | LogLevel::Debug) {
        ...
        println!("Deaths: {} ({:.1}%)", deaths, deaths as f64 / total_episodes as f64 * 100.0);
        println!("\nCheckpoints saved to: {}/", checkpoints_dir);
    } else {
        println!("{:.1} eps/sec, {:.2} avg reward, {} deaths",
            total_episodes as f64 / total_time, avg_reward, deaths);
    }
}
