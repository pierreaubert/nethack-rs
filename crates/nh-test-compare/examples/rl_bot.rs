...
use nh_test_compare::state::c_extractor::CGameWrapper;
...
impl QNetwork {
    ...
    fn update(&mut self, transitions: &[Transition], learning_rate: f64, gamma: f64) {
        for t in transitions {
            ...
            let max_next_q = if t.done { 0.0 } else { next_q_values.iter().fold(f64::MIN, |m, v| v.max(m)) };
...
impl TrainingMetrics {
    ...
    fn record_episode(&mut self, reward: f64, length: u64, died: bool, won: bool) {
        ...
        if died { self.deaths += 1; }
        if won { self.wins += 1; }
    }

    fn summary(&self) -> String {
        format!("Episodes: {}, Avg Reward: {:.2}, Avg Length: {:.1}, Deaths: {}, Wins: {}",
            self.episode_rewards.len(),
            self.episode_rewards.iter().sum::<f64>() / self.episode_rewards.len().max(1) as f64,
            self.episode_lengths.iter().sum::<u64>() as f64 / self.episode_lengths.len().max(1) as f64,
            self.deaths,
            self.wins)
    }
}
...
fn train_episode(
    ...
    c_engine.init("Tourist", "Human", 0, 0).expect("Failed to init C engine");
    ...
    while turn < 10000 {
        ...
        if new_state.is_dead { died = true; }
        if new_state.is_won { won = true; }
...
fn train_rl_bot(num_episodes: usize) {
    ...
    println!("Episodes: {}, Memory: {}, Batch: {}", num_episodes, MEMORY_SIZE, BATCH_SIZE);
    ...
    for episode in 0..num_episodes {
        ...
        if episode % 50 == 0 || episode == num_episodes - 1 {
            println!("Episode {:4}: Reward: {:7.2}, Length: {:4}, Epsilon: {:.3} | {}",
                episode, reward, length, exploration_rate, metrics.summary());
        }
    }
...
fn evaluate_bot(q_network: &QNetwork, num_episodes: usize) {
    ...
    for episode in 0..num_episodes {
        ...
        while turn < 5000 {
            ...
            if new_state.is_dead { died = true; total_deaths += 1; break; }
            if new_state.is_won { won = true; total_wins += 1; break; }
        ...
        if episode % 100 == 0 {
            println!("Episode {:4}: {} turns, {} won, {} died", episode, turn, if won { "yes" } else { "no" }, if died { "yes" } else { "no" });
        }
    }
    ...
    println!("Avg Turns per Episode: {:.1}", total_turns as f64 / num_episodes as f64);
    println!("Wins: {} ({:.1}%)", total_wins, total_wins as f64 / num_episodes as f64 * 100.0);
    println!("Deaths: {} ({:.1}%)", total_deaths, total_deaths as f64 / num_episodes as f64 * 100.0);
}
