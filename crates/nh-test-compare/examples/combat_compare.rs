...
fn run_combat_session(seed: u64, max_turns: u64) {
    ...
    for turn in 1..=max_turns {
        ...
        let action = if let Some(attack) = find_and_attack(&rust_state) {
            ...
            println!("[Turn {}] No monsters nearby, attacking {}", turn, direction_str);
            action
        };
        ...
        if rust_damage { combat_stats.rust_attacks += 1; }
        if c_damage { combat_stats.c_attacks += 1; }
        ...
        if rust_monsters != c_monsters {
            println!("  MONSTER COUNT DIFF: Rust={}, C={}", rust_monsters, c_monsters);
        ...
        if turn % 25 == 0 && combat_stats.turns > 0 {
            println!("[Turn {}] Progress: {} turns, {} HP diffs, {} monster diffs",
