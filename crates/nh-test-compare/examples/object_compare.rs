...
use nh_test_compare::state::common::GameAction;
...
impl ObjectTestResult {
    ...
    fn summary(&self) -> String {
        ...
        if self.rust_hp_after != self.c_hp_after {
            diffs.push(format!("HP: Rust={}->{}, C={}->{}",
                self.rust_hp_before, self.rust_hp_after,
                self.c_hp_before, self.c_hp_after));
        }
        if self.rust_gold != self.c_gold {
            diffs.push(format!("Gold: Rust={}, C={}", self.rust_gold, self.c_gold));
        }
        if self.rust_message != self.c_message {
            diffs.push(format!("Msg: Rust='{}' vs C='{}'",
                truncate(&self.rust_message, 50),
                truncate(&self.c_message, 50)));
        }

        format!("[DIFF] {} - {}: {}", self.object_class, self.object_name, diffs.join("; "))
    }
}
...
fn get_sample_objects() -> HashMap<ObjectClass, Vec<(&'static str, char)>> {
    ...
    // Weapons
    samples.insert(ObjectClass::Weapon, vec![
        ("dagger", ')'),
        ("long sword", ')'),
        ("axe", ')'),
    ]);
    ...
    // Armor
    samples.insert(ObjectClass::Armor, vec![
        ("leather armor", '['),
        ("chain mail", '['),
        ("shield", '['),
    ]);
    ...
    // Rings
    samples.insert(ObjectClass::Ring, vec![
        ("ring of conflict", '='),
    ...
    // Amulets
    samples.insert(ObjectClass::Amulet, vec![
    ...
    // Tools
    samples.insert(ObjectClass::Tool, vec![
        ("pick-axe", '('),
        ("sack", '('),
        ("tin opener", '('),
    ]);
    ...
    // Food
    samples.insert(ObjectClass::Food, vec![
        ("apple", '%'),
        ("carrot", '%'),
        ("dead lizard", '%'),
    ]);
    ...
    // Potions
    samples.insert(ObjectClass::Potion, vec![
        ("potion of healing", '!'),
    ...
    // Scrolls
    samples.insert(ObjectClass::Scroll, vec![
    ...
    // Spellbooks
    samples.insert(ObjectClass::Spellbook, vec![
        ("spellbook of healing", '+'),
        ("spellbook of teleportation", '+'),
    ]);

    // Wands
    samples.insert(ObjectClass::Wand, vec![
        ("wand of striking", '/'),
        ("wand of light", '/'),
        ("wand of digging", '/'),
    ]);
    ...
    // Gems
    samples.insert(ObjectClass::Gem, vec![
        ("diamond", '*'),
        ("ruby", '*'),
        ("emerald", '*'),
    ]);
