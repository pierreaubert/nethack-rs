...
use nh_test_compare::orchestrator::{DualGameOrchestrator, OrchestratorConfig, MultiSessionSummary};
...
fn run_comparison(seed: u64, max_turns: u64) {
    ...
    if result.critical_differences.is_empty() && result.major_differences.is_empty() {
        ...
        println!("Critical differences ({}):", result.critical_differences.len());
        for (turn, diff) in &result.critical_differences {
            println!("  Turn {}: {} = Rust: {:?} vs C: {:?} - {}",
