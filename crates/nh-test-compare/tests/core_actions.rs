...
use nh_core::action::eat::{
    ...
    lesshungry, newuhs, CorpseEffect, TinType,
};
...
#[test]
fn test_tin_type_nutrition_values() {
    // Verify tin types have reasonable nutrition modifiers
    assert!(TinType::Rotten.nutrition() < 0, "Rotten should be negative");
    assert!(
        TinType::Spinach.nutrition() > TinType::Homemade.nutrition(),
        "Spinach should be more nutritious than homemade"
    );
    assert!(
        TinType::Pureed.nutrition() > 100,
        "Pureed should be high nutrition"
    );
}
