...
//!
//! Given a seed and a sequence of commands, runs both C and Rust game loops
//! and compares UnifiedGameState after each turn.
//!
//! Uses the DualGameOrchestrator and compare_states() infrastructure.

use std::collections::HashMap;
use std::path::Path;

use nh_test_compare::compare::{compare_states, filter_differences, summarize_differences};
use nh_test_compare::state::common::*;
...
fn action_to_command(action: &GameAction) -> nh_core::action::Command {
    ...
    match action {
        ...
        GameAction::EatFirst => Command::Eat('a'), // first inventory slot
...
fn test_rust_replay_deterministic() {
    let replay = Replay {
        ...
        commands: parse_command_string("kjhlkjhl........"),
        tags: vec!["movement".to_string()],
    };

    // Run twice
    let rng1 = nh_core::GameRng::new(replay.seed);
    let state1 = nh_core::GameState::new(rng1);
    let mut loop1 = nh_core::GameLoop::new(state1);

    let rng2 = nh_core::GameRng::new(replay.seed);
    ...
    for action in &replay.commands {
        ...
        loop1.tick(cmd.clone());
        loop2.tick(cmd);
    }

    // Extract states and compare
    let extractor1 = nh_test_compare::state::rust_extractor::RustGameEngine::new(&mut loop1);
    let extractor2 = nh_test_compare::state::rust_extractor::RustGameEngine::new(&mut loop2);

    let state_a = extractor1.extract_state();
    let state_b = extractor2.extract_state();

    let diffs = compare_states(&state_a, &state_b);
    assert!(
        diffs.is_empty(),
        "Determinism failure: same seed + same commands produced different states.\nDiffs: {:?}",
        diffs
    );
}
