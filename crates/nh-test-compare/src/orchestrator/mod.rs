...
use crate::agent::{VirtualPlayer, VirtualPlayerConfig, SessionResult};
...
impl DualGameOrchestrator {
    ...
    pub fn run_session(
        ...
        self.player.set_exploration_rate(self.config.initial_exploration_rate);
        ...
        for turn in 0..self.config.max_turns_per_session {
            ...
            self.player.update(&rust_state, &action, combined_reward, &new_rust_state);
            ...
            if !rust_messages.is_empty() {
                result.messages.push(format!("[Turn {}] Rust: {}", turn + 1, rust_messages.join("; ")));
            }
            if !c_messages.is_empty() {
                result.messages.push(format!("[Turn {}] C: {}", turn + 1, c_messages.join("; ")));
            }
    ...
    pub fn run_multiple_sessions(
        ...
        seeds.iter()
            .map(|seed| {
                // Reset both games
                let mut new_rust_loop = nh_core::GameLoop::new(
                    nh_core::GameState::new(nh_core::GameRng::new(*seed))
                );
...
mod tests {
    ...
    fn test_quick_test_result() {
        ...
        assert_eq!(result.summary(), "Quick Test (seed=12345): 100 turns, reward=5.00, diffs=0/2 (PASSED)");
    }
