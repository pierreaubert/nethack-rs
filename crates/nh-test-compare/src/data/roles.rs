...
mod tests {
    ...
    fn test_extract_role_names() {
        ...
        assert!(names.len() >= 13, "Expected 13 roles, found {}", names.len());
    ...
    fn test_all_c_races_in_rust() {
        use nh_data::roles::RACES;

        let c_names: HashSet<String> = extract_race_names().into_iter().collect();
        let rust_names: HashSet<String> = RACES
            .iter()
            .map(|r| r.noun.to_lowercase())
            .collect();
        ...
        assert!(missing.is_empty(), "Missing {} races in Rust", missing.len());
    }
    ...
    fn test_role_rank_count() {
        ...
        for (role, ranks) in &roles_with_ranks {
            println!("  {} ({} ranks): {:?}", role, ranks.len(), &ranks[..ranks.len().min(3)]);
        }
