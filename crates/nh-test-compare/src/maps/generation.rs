...
pub fn generate_rust_level_stats(seed: u64) -> LevelStats {
    use nh_core::dungeon::{generate_rooms_and_corridors, CellType, DLevel, Level};
    use nh_core::GameRng;
        ...
        .map(|col| {
            col.iter()
                .map(|cell| match cell.typ {
                    ...
                    CellType::VWall | CellType::HWall | CellType::TLCorner |
                    CellType::TRCorner | CellType::BLCorner | CellType::BRCorner => CellCategory::Wall,
...
pub fn verify_stats_in_range(stats: &LevelStats, ranges: &StatRanges) -> Vec<String> {
    ...
    if stats.monster_count < ranges.monster_count.0
        || stats.monster_count > ranges.monster_count.1
    {
        errors.push(format!(
