...
impl DuelingQNetwork {
    ...
    fn load(&mut self, path: &str) {
        ...
        for layer in [
            ...
            &mut self.fc_value,
            &mut self.value_out,
            &mut self.fc_advantage,
            &mut self.advantage_out,
        ] {
            for row in &mut layer.weights {
                for val in row {
                    *val = values[idx];
    ...
    for _ in 0..config.num_episodes / config.num_workers {
        ...
        *episode_offset.lock().unwrap() += 1;
        ...
        let exploration_rate = (config.exploration_start
            * config
                .exploration_decay
                .powf(*episode_offset.lock().unwrap() as f64))
...
fn train_advanced_rl_bot(config: Config) {
    ...
    println!(
        "Episodes: {}, Workers: {}, Memory: {}",
        config.num_episodes, config.num_workers, config.memory_size
    );
    ...
    for batch in 0.. {
        ...
        if replay_memory.len() >= config.batch_size {
            ...
            for (i, t) in batch.iter().enumerate() {
                ...
                if idx < replay_memory.memory.len() {
                    replay_memory.memory[idx].priority =
                        (error + config.priority_epsilon).powf(config.priority_alpha);
                }
            }
        ...
        if episode % 50 == 0 && episode > 0 {
            ...
            let avg_length: f64 = batch_results.iter().map(|r| r.length as f64).sum::<f64>()
            ...
            let exploration_rate = (config.exploration_start
                * config.exploration_decay.powf(episode as f64))
            .max(config.exploration_end);

            println!(
                "Episode {:4}: Avg Reward: {:7.2}, Avg Length: {:4.0}, Deaths: {:3}, Wins: {:2}, Epsilon: {:.3} | {:.2} sec",
                episode,
                avg_reward,
                avg_length,
                deaths,
