...
fn compare_attributes(
    ...
    let attributes = [
        ...
        (
            "constitution",
            rust_state.constitution,
            c_state.constitution,
        ),
        (
            "intelligence",
            rust_state.intelligence,
            c_state.intelligence,
        ),
    ...
    for (name, rust_val, c_val) in attributes {
        if rust_val != c_val {
            differences.push(StateDifference {
                ...
                c_value: json!(c_val),
                severity: DifferenceSeverity::Minor,
                description: format!("{} mismatch: Rust {} vs C {}", name, rust_val, c_val),
            });
        }
    }
}
    ...
    if rust_state.inventory.len() != c_state.inventory.len() {
        differences.push(StateDifference {
            ...
            description: format!(
                ...
                rust_state.inventory.len(),
                c_state.inventory.len()
            ),
        });
    }
}
    ...
    if rust_state.nearby_monsters.len() != c_state.nearby_monsters.len() {
        differences.push(StateDifference {
            ...
            description: format!(
                ...
                rust_state.nearby_monsters.len(),
                c_state.nearby_monsters.len()
            ),
        });
    }
}
...
mod tests {
    ...
    fn test_position_difference() {
        ...
        assert!(
            diffs
                .iter()
                .any(|d| d.category == DifferenceCategory::Position)
        );
    }
