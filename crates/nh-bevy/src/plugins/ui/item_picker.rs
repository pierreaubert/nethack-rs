    ...
    pub fn filter(&self, obj: &Object) -> bool {
        match self {
            ...
            PickerAction::Wield => {
                ...
                || obj.class == ObjectClass::Tool // Pick-axe, etc
            }
    ...
    if list_len > 0 {
        ...
        if input.just_pressed(KeyCode::KeyK) || input.just_pressed(KeyCode::ArrowUp) {
            picker_state.selected_index =
                (picker_state.selected_index + list_len - 1) % list_len;
        }
...
fn confirm_selection(
    ...
    if let Some(action) = picker_state.action {
        match action {
            PickerAction::Eat => { commands.send(GameCommand(Command::Eat(item_char))); },
            PickerAction::Quaff => { commands.send(GameCommand(Command::Quaff(item_char))); },
            PickerAction::Read => { commands.send(GameCommand(Command::Read(item_char))); },
            PickerAction::Wield => { commands.send(GameCommand(Command::Wield(Some(item_char)))); },
            PickerAction::Wear => { commands.send(GameCommand(Command::Wear(item_char))); },
            PickerAction::TakeOff => { commands.send(GameCommand(Command::TakeOff(item_char))); },
            PickerAction::PutOn => { commands.send(GameCommand(Command::PutOn(item_char))); },
            PickerAction::Remove => { commands.send(GameCommand(Command::Remove(item_char))); },
            PickerAction::Drop => { commands.send(GameCommand(Command::Drop(item_char))); },
            PickerAction::Apply => { commands.send(GameCommand(Command::Apply(item_char))); },
...
fn render_picker(
    ...
    egui::Window::new(format!("Select item to {}", action_name))
        ...
        .show(contexts.ctx_mut(), |ui| {
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for (i, &inv_idx) in picker_state.filtered_indices.iter().enumerate() {
                            if inv_idx >= inventory.len() { continue; }
                            let item = &inventory[inv_idx];
            ...
            ui.separator();
            ui.label(egui::RichText::new("Select with arrows/j/k, Confirm with Enter").small().color(egui::Color32::GRAY));
        });
}
